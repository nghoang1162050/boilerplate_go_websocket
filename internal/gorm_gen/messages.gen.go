...
func newMessage(db *gorm.DB, opts ...gen.DOOption) message {
	...
	_message.Room = messageHasOneRoom{
...
type messageHasOneRoom struct {
	db *gorm.DB

	field.RelationField
}

func (a messageHasOneRoom) Where(conds ...field.Expr) *messageHasOneRoom {
	if len(conds) == 0 {
		return &a
	}
...
func (a messageHasOneRoom) WithContext(ctx context.Context) *messageHasOneRoom {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a messageHasOneRoom) Session(session *gorm.Session) *messageHasOneRoom {
	a.db = a.db.Session(session)
	return &a
}

func (a messageHasOneRoom) Model(m *model.Message) *messageHasOneRoomTx {
	return &messageHasOneRoomTx{a.db.Model(m).Association(a.Name())}
}

func (a messageHasOneRoom) Unscoped() *messageHasOneRoom {
	a.db = a.db.Unscoped()
	return &a
}

type messageHasOneRoomTx struct{ tx *gorm.Association }

func (a messageHasOneRoomTx) Find() (result *model.User, err error) {
	return result, a.tx.Find(&result)
}

func (a messageHasOneRoomTx) Append(values ...*model.User) (err error) {
...
func (a messageHasOneRoomTx) Replace(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a messageHasOneRoomTx) Delete(values ...*model.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a messageHasOneRoomTx) Clear() error {
	return a.tx.Clear()
}

func (a messageHasOneRoomTx) Count() int64 {
	return a.tx.Count()
}

func (a messageHasOneRoomTx) Unscoped() *messageHasOneRoomTx {
	a.tx = a.tx.Unscoped()
	return &a
}
